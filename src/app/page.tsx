    ...
    <table className="w-full max-w-[986px] hidden md:block">
      <thead>
        <tr className="border-b border-primary align-middle">
          <th colSpan={3} className="text-left text-primary text-lg">
            {benefits.headers.firstColumn}
          </th>
          <th className="bg-neutral-50 rounded-t-2xl py-8 text-center align-middle w-52">
            {benefits.headers.contabilidade}
          </th>
          <th className="text-center px-12">
            {benefits.headers.market}
          </th>
        </tr>
      </thead>
      <tbody>
        {benefits.lines.map((item, index) => (
          <tr key={index}>
            <td colSpan={3} key={index} className="font-medium">
              {item.text}
            </td>
            <td className={`dark:text-green-800 text-center bg-gray-50 ${index === benefits.lines.length - 1 ? "rounded-b-[12px]" : ""}`}>
              Incluso
            </td>
